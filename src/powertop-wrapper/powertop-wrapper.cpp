...
update_software_power_consumers(char *line, work_ctx *ctx)
{
	...
	newdescr = strndup(description, strlen(description)+1);
	...
	if (!consumer.category)
		consumer.category = strndup(category, strlen(category)+1);
...
update_device_power_report(const char *line, work_ctx *ctx)
{
	...
	newdevice = strndup(device, strlen(device)+1);
...
update_mali_gpu_power_consumers(const char *line, work_ctx *ctx)
{
	...
	if (!consumer.name)
		consumer.name = strndup(name, strlen(name)+1);

	if (!consumer.description)
		consumer.description = strndup(description, strlen(description)+1);
}
...
/*	logprintf("Thread started.");*/
	...
	oldlocale = strndup(oldlocale, strlen(oldlocale)+1);
